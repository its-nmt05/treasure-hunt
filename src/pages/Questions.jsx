...
function Questions() {
  ...
  useEffect(() => {
    ...
    databaseService
      .getPowerUpDetails(teamId)
      .then((res, error) =>
        setPowerUps(res.data.length > 0 ? res.data[0] : null)
      )
  }, [])
  ...
  return (
    <div className="space-y-12">
      <h2 className="font-bold text-[2em] text-center">
        Level : {question.level != undefined ? question.level : null} / 13
      </h2>
      <QuizCard
        teamId={teamId}
        question={question}
        ...
        hintsLeft={
          powerUps.hint_questions ? 3 - powerUps.hint_questions.length : 3
        }
        skipsLeft={
          powerUps.skipped_questions ? 1 - powerUps.skipped_questions.length : 1
        }
        hintUsed={
          powerUps.hint_questions
            ? powerUps.hint_questions.includes(question.id)
            : null
        }
      />
    </div>
